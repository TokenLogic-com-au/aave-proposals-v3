 ...
 */
contract AaveV3Ethereum_RemoteGSMLaunchMantle_20260521_Part2 is IProposalGenericExecutor {
  using SafeERC20 for IERC20;

  // GhoDirectFacilitator Constants
  // TODO: deployed Mantle-specific GhoDirectFacilitator on Ethereum
  address public constant DIRECT_FACILITATOR = address(0);
  string public constant DIRECT_FACILITATOR_NAME = 'GhoDirectFacilitator Mantle';
  ...
  // TODO: deployed AaveGhoCcipBridge on Mantle (counterpart that will receive the CCIP
  // message and forward GHO to the Mantle Collector).
  address public constant MANTLE_BRIDGE_DESTINATION = address(0);

  // TODO: confirm gas limit needed by AaveGhoCcipBridge.ccipReceive() on Mantle.
  ...
  uint32 public constant MANTLE_BRIDGE_GAS_LIMIT = 0;
  ...
  function execute() external {
    ...
    // Configure the Mantle lane on the AaveGhoCcipBridge: maps the Mantle chain
    // selector to the counterpart bridge address on Mantle (which forwards the GHO
    // to AaveV3Mantle.COLLECTOR on receipt). `extraArgs` is left empty so the bridge
    ...
    IAaveGhoCcipBridge(CCIP_BRIDGE).setDestinationChain(
      ...
      abi.encode(MANTLE_BRIDGE_DESTINATION),
      bytes(''),
      MANTLE_BRIDGE_GAS_LIMIT
    );
