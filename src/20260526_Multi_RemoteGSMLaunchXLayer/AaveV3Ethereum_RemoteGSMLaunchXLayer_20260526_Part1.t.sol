...
/**
 * @dev Test for AaveV3Ethereum_RemoteGSMLaunchXLayer_20260526_Part1
 * command: FOUNDRY_PROFILE=test forge test --match-path=src/20260526_Multi_RemoteGSMLaunchXLayer/AaveV3Ethereum_RemoteGSMLaunchXLayer_20260526_Part1.t.sol -vv
 */
contract AaveV3Ethereum_RemoteGSMLaunchXLayer_20260526_Part1_Test is ProtocolV3TestBase {
  // Existing on-chain rate-limiter values for the Eth->XLayer GHO lane.
  ...
  AaveV3Ethereum_RemoteGSMLaunchXLayer_20260526_Part1 internal proposal;

  function setUp() public {
    vm.createSelectFork(vm.rpcUrl('mainnet'), 25080900);
    proposal = new AaveV3Ethereum_RemoteGSMLaunchXLayer_20260526_Part1();
  }
  ...
  function test_defaultProposalExecution() public {
    defaultTest(
      'AaveV3Ethereum_RemoteGSMLaunchXLayer_20260526_Part1',
      AaveV3Ethereum.POOL,
      address(proposal)
    );
  }
  ...
  function test_bridgeLimit() public {
    ...
    assertEq(
      ...
      bridgeLimitBefore + RemoteGSMLaunchXLayerConstants.TEMP_BRIDGE_CAPACITY,
      'bridge limit not raised by TEMP_BRIDGE_CAPACITY after proposal'
    );
  }

  function test_rateLimiter() public {
    // Start with existing on-chain values (pre-proposal).
    ...
    assertEq(
      bucket.capacity,
      RemoteGSMLaunchXLayerConstants.TEMP_BRIDGE_CAPACITY,
      'post-proposal outbound capacity should be TEMP_BRIDGE_CAPACITY'
    );
    assertEq(
      bucket.rate,
      RemoteGSMLaunchXLayerConstants.TEMP_BRIDGE_CAPACITY - 1,
