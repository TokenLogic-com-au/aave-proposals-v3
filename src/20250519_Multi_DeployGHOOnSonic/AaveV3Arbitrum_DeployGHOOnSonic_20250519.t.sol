...
contract AaveV3Arbitrum_DeployGHOOnSonic_20250519_Test is ProtocolV3TestBase {
  ...
  uint64 internal constant GNO_CHAIN_SELECTOR = CCIPUtils.GNOSIS_CHAIN_SELECTOR;
  ...
  function _validateConstants() private view {
    assertEq(CCIPUtils.SONIC_CHAIN_SELECTOR, SONIC_CHAIN_SELECTOR);
    assertEq(address(proposal.TOKEN_POOL()), address(NEW_TOKEN_POOL));
    assertEq(proposal.REMOTE_TOKEN_POOL_SONIC(), NEW_REMOTE_POOL_SONIC);
    assertEq(proposal.REMOTE_GHO_TOKEN_SONIC(), NEW_REMOTE_TOKEN_SONIC);
    assertEq(CCIPLaunchConstants.CCIP_RATE_LIMIT_CAPACITY, CCIP_RATE_LIMIT_CAPACITY);
    assertEq(CCIPLaunchConstants.CCIP_RATE_LIMIT_REFILL_RATE, CCIP_RATE_LIMIT_REFILL_RATE);
    ...
    IInternal.EVM2EVMMessage memory eventArg = CCIPUtils.messageToEvent(
      CCIPUtils.MessageToEventParams({
        ...
        destinationToken: address(
          params.destChainSelector == SONIC_CHAIN_SELECTOR
            ? NEW_REMOTE_TOKEN_SONIC
...
contract AaveV3Arbitrum_DeployGHOOnSonic_20250519_PostExecution is
  AaveV3Arbitrum_DeployGHOOnSonic_20250519_Test
{
  ...
  function test_basePoolConfig() public view {
    ...
    assertEq(NEW_TOKEN_POOL.getSupportedChains()[2], SONIC_CHAIN_SELECTOR);
    assertEq(
    ...
    assertEq(
      NEW_TOKEN_POOL.getRemoteToken(SONIC_CHAIN_SELECTOR),
      abi.encode(address(NEW_REMOTE_TOKEN_SONIC))
    );
    ...
    assertEq(
      NEW_TOKEN_POOL.getCurrentOutboundRateLimiterState(SONIC_CHAIN_SELECTOR),
      _getRateLimiterConfig()
    );
  }
  ...
  function test_sendMessageToSonicSucceeds(uint256 amount) public {
    ...
    (
      ...
      IInternal.EVM2EVMMessage memory eventArg
        ...
        CCIPSendParams({amount: amount, sender: alice, destChainSelector: SONIC_CHAIN_SELECTOR})
      );

    vm.expectEmit(address(NEW_TOKEN_POOL));
    emit Burned(address(SONIC_ON_RAMP), amount);
    vm.expectEmit(address(SONIC_ON_RAMP));
    emit CCIPSendRequested(eventArg);

    vm.prank(alice);
    ROUTER.ccipSend{value: eventArg.feeTokenAmount}(SONIC_CHAIN_SELECTOR, message);
  ...
  function test_offRampViaSonicSucceeds(uint256 amount) public {
    ...
    vm.expectEmit(address(NEW_TOKEN_POOL));
    emit Minted(address(SONIC_OFF_RAMP), alice, amount);

    vm.prank(address(SONIC_OFF_RAMP));
    NEW_TOKEN_POOL.releaseOrMint(
      IPool_CCIP.ReleaseOrMintInV1({
        originalSender: abi.encode(alice),
        remoteChainSelector: SONIC_CHAIN_SELECTOR,
        receiver: alice,
        amount: amount,
  ...
  function test_cannotUseBaseOffRampForEthMessages() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(CallerIsNotARampOnRouter.selector, address(SONIC_OFF_RAMP))
    );
    vm.prank(address(SONIC_OFF_RAMP));
    NEW_TOKEN_POOL.releaseOrMint(
      IPool_CCIP.ReleaseOrMintInV1({
